...
| **Message Consumer** | Async message triggers Application Service, which coordinates Aggregate behavior and Domain Event publication |
...
All modules register on a shared `ARCHV` object (`window.ARCHV`). Each architecture file creates a namespace (e.g., `ARCHV.layered`, `ARCHV.clean`) with a `modes` array, `depRules` array, and mode objects. Each mode object exposes two methods:
...
DOMContentLoaded
  ...
  -> readHash() || switchArch('layered')   # restore from URL hash or default
...
| `ARCHV.renderComponent()` | Component renderer | Generates `.archv-component` spans with icon and name |
...
│   ├── ddd.js              # 3 modes: HTTP, Console, Message — DDD tactical patterns
...
2. `layered.js` — registers `ARCHV.layered` with modes and rendering functions
3. `clean.js` — registers `ARCHV.clean` with concentric ring visualization
4. `hexagonal.js` — registers `ARCHV.hexagonal` with ports & adapters layout
5. `ddd.js` — registers `ARCHV.ddd` with DDD tactical patterns
