// Learn page: Daily 10, Flashcards (Leitner), Quiz, Browse over KNOWLEDGE_DATA
// (generated by scripts/extract_knowledge.py into resources/knowledge-data.js).
...
function renderCard(item, options) {
    ...
    if (options && options.onDone) {
        ...
        label.className = 'k-done';
        const checkbox = document.createElement('input');
        checkbox.type = 'checkbox';
        checkbox.checked = Boolean(options.done);
        checkbox.addEventListener('change', () => options.onDone(checkbox.checked));
        label.appendChild(checkbox);
        label.appendChild(document.createTextNode('done'));
        head.appendChild(label);
        if (options.done) card.classList.add('done');
    }
...
function initDaily() {
    ...
    dailyState = loadState('learnDailyState');
    if (!dailyState || dailyState.date !== todayStr()) {
        dailyState = { v: 1, date: todayStr(), done: [] };
    }
...
function renderDaily() {
    ...
    shown.forEach(item => {
        fragment.appendChild(renderCard(item, {
            done: dailyState.done.indexOf(item.id) !== -1,
            onDone: checked => {
                const index = dailyState.done.indexOf(item.id);
                if (checked && index === -1) dailyState.done.push(item.id);
                if (!checked && index !== -1) dailyState.done.splice(index, 1);
                saveState('learnDailyState', dailyState);
                renderDaily();
            }
        }));
    });
    list.appendChild(fragment);
    const doneCount = shown.filter(item => dailyState.done.indexOf(item.id) !== -1).length;
    document.getElementById('daily-stats').textContent =
        `${todayStr()} · done ${doneCount}/${shown.length}`;
    document.getElementById('daily-bar').style.width =
        shown.length ? (100 * doneCount / shown.length) + '%' : '0';
...
function initCards() {
    cardsState = loadState('learnCardsState') || { v: 1, cards: {} };
    buildCardsQueue();
...
function initQuiz() {
    quizStats = loadState('learnQuizStats') || {
        ...
        byType: { term: { a: 0, c: 0 }, quote: { a: 0, c: 0 }, principle: { a: 0, c: 0 } }
    };
...
document.addEventListener('DOMContentLoaded', () => {
    if (typeof KNOWLEDGE_DATA === 'undefined' || typeof KNOWLEDGE_META === 'undefined') {
    ...
    document.getElementById('learn-intro').textContent =
        ...
        `extracted from Life Principles and ${Object.keys(KNOWLEDGE_META.sources).length - 1} book reviews. ` +
        `Updated ${KNOWLEDGE_META.generated}.`;
